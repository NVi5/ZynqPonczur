...
extern "C" {
	volatile uint8_t framebuffer[2][800*600];
}
...
int32_t test_software[sizeof(test)/4];
uint32_t len_software = sizeof(test_software)/4;
...
int new_model = 0;
typedef enum {HARDWARE, SOFTWARE} renderer_mode_t;
renderer_mode_t renderer_mode = HARDWARE;
...
void onMessage (websocket_t *websocket, websocket_frame_t * frame){
	if (frame->opcode == websocket_opcode_binary) {
		...
		if (!strcmp("software", (char*)frame->data)) {
			renderer_mode = SOFTWARE;
		}
		else if  (!strcmp("hardware", (char*)frame->data)) {
	...
	while (1) {
		...
		if (websocket->status == websocket_status_open) {
			if (data_size <= 0){
				...
				if (renderer_mode == HARDWARE) {
				...
				else if (renderer_mode == SOFTWARE) {
					if (current_framebuffer == 0) current_framebuffer = 1;
					else current_framebuffer = 0;
					int32_t transform_matrix[4][4];
					glm::mat4 Model = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 0.0f));
					Model = glm::scale(Model, glm::vec3(scale_x,scale_y,1.0f));
					Model = glm::rotate(Model, alpha, glm::vec3(1.0f, 0.0f, 0.0f));
					Model = glm::rotate(Model, beta, glm::vec3(0.0f, 1.0f, 0.0f));
					...
					for (int i = 0; i < 4; i++) {
						for (int j = 0; j < 4; j++) {
							transform_matrix[i][j] = (int32_t)(Model[j][i]*128.0f);
						}
					}
					for (unsigned int i = 0; i < len_software; i+=4){
						vertex_process_software(&test[i], &test_software[i], transform_matrix);
					}
					rasterize_software(test_software, (uint8_t*)framebuffer[current_framebuffer], len_software);
